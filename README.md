...
然后，使用 `getopt.h` 中的 `getopt()` 函数来提取命令行选项和选项对应的参数（如有），同时按照接口规范设置上述全局变量的值。在处理完命令行选项后，处理剩余的命令行参数并同样设置全局变量。
...
在需要创建一个词法单元的时候，调用 `new_token` 函数，它接受行号，词法单元名，词法属性值，返回一个组织好的词法单元结构体。
...
用一个状态变量来保存当前状态，根据当前状态和 `fgetc` 获取的字符，利用转移函数来决定下一步将要转移到哪个状态。（需要注意的是，需要在途中遇到换行符时增加行号）：
...
token_t* getToken(FILE *fp, int *line) {
    ...
    switch (state) {
        ...
        case START:
            return NULL;
        // POSIX 标准要求所有文件的 EOF 前必须有一个换行符，因此如果是其他状态，说明该文件不符合 POSIX 标准，抛出错误（当然文件末尾没有换行并不会一定导致出错）
        default:
...
终结符节点可以直接沿用 Token，而符号节点则需要根据符号名、行号、和各个子节点的列表进行创建，如下所示：
...
syntax_t* new_symbol(char* name, int lineno, int size, ...) {
    ...
    ret->symbol.child = malloc(size * sizeof(syntax_t*));  // 分配内存存储子节点
...
- params -> void | param_list
  这个符号比较特殊，可以先判断下个终结符是否是 void：若不是，则肯定是 param_list；若是，则 void 是 param_list 的前缀，按照前述方法判断再下个是不是右括号：如果不是，则肯定不是 void，因为 params 不能接续右括号；如果是，则肯定是 void，因为 params 在 void 以后不能以 ) 开头。
  ```c
  syntax_t* params(bool last) {
    SAVE_CONT;
...
- simple_expression -> additive_expression | additive_expression relop additive_expression
  本产生式不能以 relop 能够推导的几个终结符结尾。因此如果在匹配完第一个 additive_expression 之后，如果出现一个 relop，则必须使用更长的产生式，否则显然也不能使用长的产生式。
  ```c
  syntax_t* simple_expression(bool last) {
    SAVE_CONT;
...
Token {name: ASSIGN, line: 4, value: =}
Token {name: ID, line: 4, value: a}
Token {name: PLUS, line: 4, value: +}
Token {name: ID, line: 4, value: b}
Token {name: SEMI, line: 4, value: ;}
Token {name: RETURN, line: 5, value: return}
Token {name: ID, line: 5, value: c}
Token {name: SEMI, line: 5, value: ;}
Token {name: RC, line: 6, value: }}
Token {name: TYPE, line: 9, value: int}
Token {name: ID, line: 9, value: samplenumber}
Token {name: SEMI, line: 9, value: ;}
Token {name: TYPE, line: 11, value: int}
Token {name: ID, line: 11, value: samplearray}
Token {name: LB, line: 11, value: [}
Token {name: INT, line: 11, value: 10}
Token {name: RB, line: 11, value: ]}
Token {name: SEMI, line: 11, value: ;}
Token {name: TYPE, line: 13, value: int}
Token {name: ID, line: 13, value: main}
Token {name: LP, line: 13, value: (}
Token {name: TYPE, line: 13, value: void}
Token {name: RP, line: 13, value: )}
Token {name: LC, line: 13, value: {}
Token {name: TYPE, line: 15, value: int}
Token {name: ID, line: 15, value: a}
Token {name: LB, line: 15, value: [}
Token {name: INT, line: 15, value: 10}
Token {name: RB, line: 15, value: ]}
Token {name: SEMI, line: 15, value: ;}
Token {name: TYPE, line: 16, value: int}
Token {name: ID, line: 16, value: b}
Token {name: SEMI, line: 16, value: ;}
Token {name: TYPE, line: 17, value: int}
Token {name: ID, line: 17, value: c}
Token {name: SEMI, line: 17, value: ;}
Token {name: ID, line: 19, value: a}
Token {name: LB, line: 19, value: [}
Token {name: INT, line: 19, value: 3}
Token {name: RB, line: 19, value: ]}
Token {name: ASSIGN, line: 19, value: =}
Token {name: INT, line: 19, value: 5}
Token {name: SEMI, line: 19, value: ;}
Token {name: ID, line: 21, value: b}
Token {name: ASSIGN, line: 21, value: =}
Token {name: LP, line: 21, value: (}
Token {name: INT, line: 21, value: 1}
Token {name: PLUS, line: 21, value: +}
Token {name: INT, line: 21, value: 9}
Token {name: STAR, line: 21, value: *}
Token {name: INT, line: 21, value: 7}
Token {name: RP, line: 21, value: )}
Token {name: STAR, line: 21, value: *}
Token {name: LP, line: 21, value: (}
Token {name: INT, line: 21, value: 2}
Token {name: PLUS, line: 21, value: +}
Token {name: ID, line: 21, value: a}
Token {name: LB, line: 21, value: [}
Token {name: INT, line: 21, value: 1}
Token {name: RB, line: 21, value: ]}
Token {name: LESS, line: 21, value: <}
Token {name: INT, line: 21, value: 3}
Token {name: RP, line: 21, value: )}
Token {name: SEMI, line: 21, value: ;}
Token {name: LC, line: 23, value: {}
Token {name: ID, line: 23, value: c}
Token {name: ASSIGN, line: 23, value: =}
Token {name: ID, line: 23, value: samplefunc}
Token {name: LP, line: 23, value: (}
Token {name: LP, line: 23, value: (}
Token {name: ID, line: 23, value: a}
Token {name: LB, line: 23, value: [}
Token {name: INT, line: 23, value: 1}
Token {name: RB, line: 23, value: ]}
Token {name: PLUS, line: 23, value: +}
Token {name: INT, line: 23, value: 45}
Token {name: RP, line: 23, value: )}
Token {name: COMMA, line: 23, value: ,}
Token {name: ID, line: 23, value: b}
Token {name: RP, line: 23, value: )}
Token {name: SEMI, line: 23, value: ;}
Token {name: RC, line: 23, value: }}
Token {name: LC, line: 24, value: {}
Token {name: SEMI, line: 24, value: ;}
Token {name: SEMI, line: 24, value: ;}
Token {name: RC, line: 24, value: }}
Token {name: LC, line: 25, value: {}
Token {name: LC, line: 25, value: {}
Token {name: RC, line: 27, value: }}
Token {name: RC, line: 27, value: }}
Token {name: IF, line: 29, value: if}
Token {name: LP, line: 29, value: (}
Token {name: ID, line: 29, value: samplefunc}
Token {name: LP, line: 29, value: (}
Token {name: INT, line: 29, value: 4}
Token {name: COMMA, line: 29, value: ,}
Token {name: INT, line: 29, value: 1}
Token {name: EQUAL, line: 29, value: ==}
Token {name: INT, line: 29, value: 4}
Token {name: RP, line: 29, value: )}
Token {name: GREAT, line: 29, value: >}
Token {name: ID, line: 29, value: a}
Token {name: LB, line: 29, value: [}
Token {name: INT, line: 29, value: 4}
Token {name: RB, line: 29, value: ]}
Token {name: RP, line: 29, value: )}
Token {name: IF, line: 30, value: if}
Token {name: LP, line: 30, value: (}
Token {name: ID, line: 30, value: a}
Token {name: LB, line: 30, value: [}
Token {name: INT, line: 30, value: 3}
Token {name: RB, line: 30, value: ]}
Token {name: NEQ, line: 30, value: !=}
Token {name: ID, line: 30, value: a}
Token {name: LB, line: 30, value: [}
Token {name: INT, line: 30, value: 5}
Token {name: RB, line: 30, value: ]}
Token {name: RP, line: 30, value: )}
Token {name: LC, line: 30, value: {}
Token {name: WHILE, line: 32, value: while}
Token {name: LP, line: 32, value: (}
Token {name: ID, line: 32, value: a}
Token {name: LB, line: 32, value: [}
Token {name: INT, line: 32, value: 4}
Token {name: RB, line: 32, value: ]}
Token {name: RP, line: 32, value: )}
Token {name: LC, line: 32, value: {}
Token {name: ID, line: 33, value: a}
Token {name: LB, line: 33, value: [}
Token {name: INT, line: 33, value: 4}
Token {name: RB, line: 33, value: ]}
Token {name: ASSIGN, line: 33, value: =}
Token {name: ID, line: 33, value: a}
Token {name: LB, line: 33, value: [}
Token {name: INT, line: 33, value: 4}
Token {name: RB, line: 33, value: ]}
Token {name: MINUS, line: 33, value: -}
Token {name: INT, line: 33, value: 1}
Token {name: SEMI, line: 33, value: ;}
Token {name: RC, line: 34, value: }}
Token {name: RC, line: 35, value: }}
Token {name: ELSE, line: 35, value: else}
Token {name: LC, line: 35, value: {}
Token {name: LC, line: 36, value: {}
Token {name: INT, line: 36, value: 0}
Token {name: SEMI, line: 36, value: ;}
Token {name: RC, line: 36, value: }}
Token {name: RC, line: 37, value: }}
Token {name: RETURN, line: 39, value: return}
Token {name: ID, line: 39, value: a}
Token {name: LB, line: 39, value: [}
Token {name: INT, line: 39, value: 9}
Token {name: RB, line: 39, value: ]}
Token {name: STAR, line: 39, value: *}
Token {name: LP, line: 39, value: (}
Token {name: INT, line: 39, value: 2}
Token {name: PLUS, line: 39, value: +}
Token {name: INT, line: 39, value: 3}
Token {name: RP, line: 39, value: )}
Token {name: SEMI, line: 39, value: ;}
Token {name: RC, line: 40, value: }}
```
              ...
              compound_stmt (13)
                ...
                local_declarations (15)
                  param (15)
                    ...
                    RB: ]
                    SEMI: ;
                  local_declarations (16)
                    param (16)
                      TYPE: int
                      ID: b
                      SEMI: ;
                    local_declarations (17)
                      param (17)
                        TYPE: int
                        ID: c
                        SEMI: ;
                      ...
                      statement_list (24)
                        ...
                        statement_list (25)
                          statement (25)
                            compound_stmt (25)
                              LC: {
                              local_declarations (25)
                              statement_list (25)
                                statement (25)
                                  compound_stmt (25)
                                    LC: {
                                    local_declarations (27)
                                    statement_list (27)
                                    RC: }
                              RC: }
                          statement_list (29)
                            statement (29)
                              selection-statement (29)
                                IF: if
                                LP: (
                                expression (29)
